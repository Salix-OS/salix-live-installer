...
# CONFIGURATION APPLY BUTTONS ###
		...
		for set in NewLinValues :
			if set[1] not in UnsetValues :
				if set[2] not in UnsetValues :
					LinPartitionLabel.append(set[0] + " will be formatted with " + set[1] + 
					" and will be mounted as " + set[2] + ". \n")
    ...
    def on_windows_partition_apply_clicked(self, widget, data=None):
		...
		for set in NewWinValues :
			if set[1] in UnsetValues :
				...
				WinPartitionLabel.append(set[0] + " will not be formatted and will be mounted as " 
				+ set[1] + ". \n")
    ...
    def on_users_apply_clicked(self, widget, data=None):
		...
		if self.UserLoginEntry.get_text() == '' :
			error_dialog("\nYour user's name is empty. \n\nPlease verify and correct! \n")
		elif self.UserLoginEntry.get_text().replace(' ', '').isalnum() == False :
			error_dialog("""\nYour user's name should only contain alphanumeric characters. 
\nPlease verify and correct! \n""")	
		elif ' ' in self.UserLoginEntry.get_text() :
			error_dialog("\nYour user's name should not contain any space. \n\nPlease verify and correct! \n")
		elif self.UserLoginEntry.get_text().islower() != True :
			error_dialog("\nYour user's name should not contain any upper case letter. \n\nPlease verify and correct! \n")
		elif self.UserPass1Entry.get_text() == '' :
			error_dialog("\nYour password entry is empty. \n\nPlease verify and correct! \n")			
		elif len(self.UserPass1Entry.get_text()) < 5 :
			error_dialog("""\nYour password is too short. It should have at least 5 characters. 
\nPlease verify and correct! \n""")			
		elif self.UserPass1Entry.get_text() != self.UserPass2Entry.get_text() :
			error_dialog("\nYour 2 password entries do not match. \n\nPlease verify and correct! \n") 
		else :
			global NewUser
...
\nPlease verify and correct! \n""")	
    ...
    def on_install_button_clicked(self, widget, data=None):
		...
		LastRecapFullText = ''
		LastRecapFullText += "\n<b>You are about to install Salix with the following settings:</b> \n"
		LastRecapFullText += "\n<b>Keyboard layout:</b> \n" + Selected_Keyboard + "\n"
		LastRecapFullText += "\n<b>System language:</b> \n" + Selected_Locale + "\n"
		LastRecapFullText += "\n<b>Partitions:</b> \n"
		LastRecapFullText += Selected_Main_Partition + " will be formatted with " + Selected_Main_Format + " and will be mounted as / \n"
		if LinFullSets != [] :
			for i in LinFullSets :
				LastRecapFullText += i[0] + " will be formatted with " + i[1] + " and will be mounted as " + i[2] + "\n"
		if LinFormatSets != [] :
			for i in LinFormatSets :
				LastRecapFullText += i[0] + " will be formatted with " + i[1] + " but will not be mounted \n"
		if LinMountSets != [] :
			for i in LinMountSets :
				LastRecapFullText += i[0] + " will not be formatted and will be mounted as " + i[2] + "\n"	
		if WinMountSets != [] :
			for i in WinMountSets :
				LastRecapFullText += i[0] + " will not be formatted and will be mounted as " + i[1] + "\n"	
		LastRecapFullText += "\n<b>Standard User:</b> \n" + NewUser + "\n"
		LastRecapFullText += "\n<b>Packages:</b> \n"
		LastRecapFullText += " You have chosen the " + Selected_Install_Mode + " installation mode with the " + Selected_Kernel + " kernel \n"
